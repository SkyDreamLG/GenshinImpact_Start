...
if is_admin():
    path = ''  # 初始化文件路径变量
    print('运行本程序前请确保原神图像设置已设置为独占全屏，否则会导致效果不佳')
    input('按回车键继续')
    ...
    def path_set():
        with open('yuanshen_path.txt', 'w') as file_object:
    ...
    try:
        # 打开路径记录文件
        with open('yuanshen_path.txt') as file_object:
            path_0 = file_object.read()  # 读取文件
    ...
    # 原神，启动！
    os.startfile(r"E:\Genshin Impact Game\YuanShen.exe")
